...
def predict(text): 
    ...
    if text!="":
        ...
        testing_padded = mp.array(padded)
        predicted_class_label=mp.argmax(model.predict(testing_padded),axis = 1)
        print(predicted_class_label)
        for key,value in emo_code_url.items():
            if value[0]==predicted_class_label:
                predicted_emotions_img_url = value[1]
                predicted_emotion = key
        return predicted_emotion,prediced_emotion_img_url
